...
3. **Cognitive Planning:** `GAMBIT` digests the request and generates a discrete, multi-step `ExecutionPlan`.
...
- **GAMBIT:** Exclusively responsible for cognitive planning and decomposition.
...
## Memory Architecture
The cognitive storage system (`MemoryManager`) utilizes an SQLite persistence store. It provides key-value storage enhanced with relevance scoring and categorical indexing to empower deterministic search and retrieval.
...
## Metrics
Deterministic telemetry strictly records system operations without external infrastructure (e.g., Prometheus). `OrchestratorMetricsCollector`, `RouterMetricsCollector`, `WorkflowMetricsCollector`, `ToolMetricsCollector`, and `ProviderMetrics` track operational successes, latencies, and failures in memory.

## Testing Strategy
The architecture guarantees integrity via a 181-test regression suite. Test cases systematically validate isolated subsystem boundaries, partial execution failure modes, metric accrual correctness, and circular dependency prevention.
...
3. **No Autonomous Loops in Workflow:** `WorkflowEngine` strictly sequentially executes. Goal-directed loops must exist at the agent level (Phase 3).

## Current Constraints
- Subsystem Metrics currently lack a unified observable base contract.
- Tools are currently limited to fundamental system functions (e.g., filesystem).
- Search remains fundamentally keyword-driven.

## Future Work
With infrastructure solidly defined in Phase 2, Phase 3 will introduce dynamic reflection, autonomous agents, semantic vector memory, and self-correcting goal progression.
